...
namespace UltimateIsometricToolkit.Editor
{
	...
	public static class EditorHelpers
	{
		...
		private static void SetProjection(Isometric.Projection projection) {
			...
			for (int i = 0; i < 4; i++)
				UnityEditor.Menu.SetChecked("Tools/UIT/Projection/" + (Isometric.Projection) i, false);
			UnityEditor.Menu.SetChecked("Tools/UIT/Projection/" + projection, true);
		...
		public static void ApplyProjection(bool enabled)
		{
			...
			UnityEditor.Menu.SetChecked(projectionEnabledEditorPrefsKey, _enabled);
