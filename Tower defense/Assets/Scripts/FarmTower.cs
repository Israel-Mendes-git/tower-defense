...
public class FarmTower : TowerBase
{
    ...
    [SerializeField, Range(0f, 0.5f)] private float interestRate = 0.05f; // sobre o saldo, com a habilidade
    [SerializeField] private int interestCap = 300;                       // teto por rodada, evita bola de neve
    ...
    protected override void DefinePaths(List<UpgradeTier> a, List<UpgradeTier> b)
    {
        ...
        b.Add(new UpgradeTier("BANCO CENTRAL", "Juros muito maiores e sem teto por rodada.",
    ...
    private float CurrentInterest => HasAbility("centralbank") ? interestRate * 2.5f : interestRate;
    private int CurrentCap => HasAbility("centralbank") ? int.MaxValue : interestCap;
    ...
    public override string GetStatsText()
    {
        string juros = PaysInterest
            ? $"  ·  +{CurrentInterest * 100f:0}% do caixa" + (CurrentCap == int.MaxValue ? " (sem teto)" : $" (máx ${CurrentCap})")
            : "";
